...
parser.add_argument('-dataset','--dataset', type=str, default="Song", help='dataset on which the experiment is conducted')
...
parser.add_argument('-hs', '--hidden_size', type=int, default=512, help='the dimensionality of hidden embeddings.')
...
def main(args,nfold):
    ...
    for fold in range(0,nfold):
        ...
        isValid = torch.nonzero((Y.unsqueeze(1) == valid_cp).all(dim=-1))          # (<m,num_valid)
        Y_valid = torch.zeros(Y.size(0),dtype=torch.long) + configs['num_valid']   # (m,)
        Y_valid = Y_valid.to(device)
        Y_valid[isValid[:,0]] = isValid[:,1]                                       # (m,)
        Y_valid_train = Y_valid[train_idx]
        Y_valid_test = Y_valid[test_idx]
...
def predict(test_loader, model, configs, Y_valid_test, criterion_list, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (X, Y) in enumerate(test_loader):
            ...
            batchsize = Y.size(0)
            num_dim = Y.size(1)
            Z, X_hat, pred_probs, joint_probs = model(X)
            loss_ae = criterion_list[0](X_hat,X) 
            # loss_ae /= batchsize
            loss_cls = torch.tensor(0, dtype=torch.float32).to(device)
            for dim, prob in enumerate(pred_probs):
